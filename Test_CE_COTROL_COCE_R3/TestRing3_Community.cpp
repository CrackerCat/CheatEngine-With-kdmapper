...
#define IOCTL_CE_ALLOCATE_MEMORY_FOR_DBVM		CTL_CODE(IOCTL_UNKNOWN_BASE, 0x0862, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)#define IOCTL_UNKNOWN_BASE					FILE_DEVICE_UNKNOWN
...
int main()
{
	...
	HANDLE hDevice = CreateFile(
		...
		NULL,                        // 默认安全属性 
		OPEN_EXISTING,               // 必须为 OPEN_EXISTING 
		FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, // 同步或异步标志 
		NULL
	);
...
//input.dbvmimgpath : = qword(ptrUint(@temp[1]));
	...
	inputpath ip;
	ip.cpuid = 0x1234;
	const WCHAR* a= L"C:\\Users\\Administrator\\Desktop\\bin\\vmdisk.img";
	ip.dbvmpath = (ULONG64)a;
	inputBuffer.ControlCode = IOCTL_CE_LAUNCHDBVM;

	inputBuffer.inputBuffer = &ip;
	BOOL result = DeviceIoControl(
		hDevice,
		0,
		&inputBuffer, sizeof(inputBuffer.inputBuffer),  // 输入缓冲区 
		&outputBuffer, sizeof(outputBuffer),// 输出缓冲区 
		&bytesReturned,
		NULL
	);

	printf("out put buff %I64x", outputBuffer);
	CloseHandle(hDevice);
	system("pause");
	return 0;
}
