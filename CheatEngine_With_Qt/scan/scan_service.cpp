...
ScanService::ScanService(QObject *parent)
    : QObject(parent),
      m_processSnapshotManager(
          ProcessManager::instance().getProcessMemorySnapshotManager()),
      m_engine(std::make_unique<ScanEngine>(m_processSnapshotManager.get())),
      m_repository(std::make_unique<ScanResultRepository>()),
      m_dataProvider(std::make_unique<ScanDataProvider>(
          m_processSnapshotManager.get(), ScanDataType::Int32)),
      m_viewModel(std::make_unique<ScanResultViewModel>(
          m_repository.get(), m_dataProvider.get(), this)),
...
void ScanService::startScan(const ScanRequest &request) {
  ...
  QMetaObject::invokeMethod(
      this,
      [this, pack, mode = request.mode] { this->onScanFinished(pack, mode); },
      Qt::QueuedConnection);
#else
  // 【修改】使用独立的线程运行 execute，不占用线程池名额
  std::thread([this, request, currentResults]() {
    auto pack =
        m_engine->execute(request, currentResults); // 内部会调用 dispatchScan
    ...
    QMetaObject::invokeMethod(
        ...
        [this, pack, mode = request.mode] { this->onScanFinished(pack, mode); },
        Qt::QueuedConnection);
  }).detach(); // 这里的管理线程可以安全阻塞，因为它不属于
               // GlobalThreadPool,GlobalThreadPool里的线程会被计算密集的task占满，使用同一个GlobalThreadPool会发生线程池饥饿
...
bool ScanService::isScanning() const { return m_scanning.load(); }

bool ScanService::hasResults() const {
  // UnknownInitial 模式（无论是否正在扫描）：认为有结果
  if (m_expectEmptyResults.load())
    return true;
...
void ScanService::stopAutoRefresh() { m_refreshTimer->stop(); }
...
bool ScanService::restorePreviousResults() {
  if (!m_repository)
    return false;
  if (!m_repository->swapWithPrevious())
    return false;
