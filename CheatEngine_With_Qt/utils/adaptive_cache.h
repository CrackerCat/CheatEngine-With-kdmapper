...
template <typename T> class AdaptiveCache {
  static_assert(std::is_trivially_copyable_v<T>,
                "AdaptiveCache requires trivially copyable types.");

public:
  // 【修改】构造函数增加唯一标识，防止多线程创建文件冲突
  explicit AdaptiveCache(size_t memThreshold = 100'000,
  ...
  void push_back_batch(const std::vector<T> &items) {
    ...
    if (m_onDisk || (m_buffer.size() + items.size() >= m_threshold)) {
      if (!m_onDisk)
        flushToDisk(); // 第一次超过阈值，把内存里的全倒进去

      ensureFileOpen();
      m_diskOut.write(reinterpret_cast<const char *>(items.data()),
                      items.size() * sizeof(T));
...
private:
  ...
  void ensureFileOpen() {
    if (!m_diskOut.is_open()) {
      if (m_diskPath.empty()) {
        ...
        // 【修改】文件名包含唯一标识，避免冲突
        m_diskPath =
            (dir / ("ACache_" + m_uniqueId + "_" + std::to_string(ts) + ".tmp"))
                .string();
      }
  ...
  void flushToDisk() {
    if (m_buffer.empty())
      return;
    ensureFileOpen();
    m_diskOut.write(reinterpret_cast<const char *>(m_buffer.data()),
...
// =================================================================
template <typename T> class AdaptiveCachePool {
public:
  ...
  void push_back(const T &item) { getThreadLocalCache().push_back(item); }
    ...
    for (const auto &[id, cache] : m_subCaches) {
      ...
      if (start < currentOffset + cacheSize) {
        size_t internalStart =
            (start > currentOffset) ? (start - currentOffset) : 0;
        size_t internalCount =
            std::min(remainingToRead, cacheSize - internalStart);
...
private:
  ...
  //         实例时，才允许走极速无锁路径 if (lastSeenPoolId == this) [[likely]]
  //         {
  ...
  //             auto newCache =
  //             std::make_unique<AdaptiveCache<T>>(m_perThreadThreshold, tid);
